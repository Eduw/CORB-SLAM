...
int main(int argc, char **argv)
{
    ...
    cv::Mat im;
    cv::Mat im2;
    for(int ni=0; ni<674+2760-1384; ni++)
    {
        ...
        if(ni>674) {
            // Read image from file
            im2 = cv::imread(vstrImageFilenames[ni+1384-674], CV_LOAD_IMAGE_UNCHANGED);
            tframe2 = vTimestamps[ni+1384-674];

            if (im2.empty()) {
                cerr << endl << "Failed to load image 2 at: " << vstrImageFilenames[ni+1384-674] << endl;
                return 1;
            }
        }
    ...
    float totaltime = 0;
    for(int ni=0; ni<1384-674; ni++)
    {
        totaltime+=vTimesTrack[ni];
    }
    cout << "-------" << endl << endl;
    cout << "median tracking time: " << vTimesTrack[(1384-674)/2] << endl;
    cout << "mean tracking time: " << totaltime/(1384-674) << endl;
