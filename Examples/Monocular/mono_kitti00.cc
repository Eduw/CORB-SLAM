...
int main(int argc, char **argv)
{
    ...
    for(int ni=0; ni<offset; ni++)
    {
        ...
        // Read image from file
        im2 = cv::imread(vstrImageFilenames[ni+1960],CV_LOAD_IMAGE_UNCHANGED);
        double tframe2 = vTimestamps[ni+1960];

        if(im2.empty())
        {
            cerr << endl << "Failed to load image 2 at: " << vstrImageFilenames[ni+1960] << endl;
            return 1;
        }
        ...
        double T=0;
        if(ni<nImages-1)
            T = (vTimestamps[ni+1]-tframe)*1;
        else if(ni>0)
            T = (tframe-vTimestamps[ni-1])*1;
        //W Times 10

        if(ttrack<T)
            usleep((T-ttrack)*1e6);
//        else
//            cout << "Tracking is taking too long..." << ni << endl;
    }
